...
Answer: Number of conflicts per year
...
Whether the mine was active, before during and after. - numerical

type of mine - categorical
...
```{r}
finalish <- final %>%
  mutate(num_open= year_de - year_do + 1) 

ggplot(data = finalish, mapping = aes(x= num_open , y= n )) + geom_point() + geom_smooth(method="lm", se=FALSE)
...
```{r}
output <- lm(n~ num_open, data = finalish)
 output
```
...
$Y_{it}$ represent the outcome variable \_\_num of conflicts\_\_ for each \_\_mine\_\_ (geographical unit) $i$, and \_\_year\_\_\_ (time unit) $t$.

Based on the regression from Question 6, $\beta_0$ is equal to \_\_0.13832\_\_\_.

Based on the regression from Question 6, $\beta_1$ is equal to \_\_\_0.01739\_\_.
